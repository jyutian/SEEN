...
class AudioDenoisePipeline:
    ...
    def _run(self, audio_path, text):
        ...
        user_question = {'role': 'user', 'content': [text, librosa.load(audio_path, sr=16000, mono=True)[0]]}
...
def read_csv(csv_path,test_samples=100):
    ...
    for i, row in tqdm(df.iterrows(), total=min(test_samples, len(df))):
        ...
        text_test.append(row["question"]+row["choices"]+"Answer the multiple-choice question by outputting ONLY one letter.")
...
if __name__ == "__main__":
    ...
    y_pred_noise = (df["res_noise"] == df["res"]).astype(int).value
